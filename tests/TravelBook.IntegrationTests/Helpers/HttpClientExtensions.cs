...
namespace TravelBook.IntegrationTests.Helpers;
...
public static class HttpClientExtensions
{
        ...
        IEnumerable<KeyValuePair<string, string>> formValues)
    {
        foreach (var kvp in formValues)
        {
            ...
            try
            {
                input = Assert.IsAssignableFrom<IHtmlInputElement>(form[kvp.Key]);
                input.Value = kvp.Value;
            }
            catch (Xunit.Sdk.IsAssignableFromException)
            {
                textarea = Assert.IsAssignableFrom<IHtmlTextAreaElement>(form[kvp.Key]);
